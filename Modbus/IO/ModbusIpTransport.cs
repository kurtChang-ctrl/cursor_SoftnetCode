...
namespace Modbus.IO
{
	...
	internal class ModbusIpTransport : ModbusTransport
	{
		...
		internal static byte[] ReadRequestResponse(IStreamResource streamResource)
		{
            ...
            if (LogUtility.IsEnable) LogUtility.Debug(string.Format("MBAP header: {0}", StringUtility.Join(", ", mbapHeader)), "ReadRequestResponse");
            ...
            if (LogUtility.IsEnable) LogUtility.Debug(string.Format("{0} bytes in PDU.", frameLength), "ReadRequestResponse");
            ...
            if (LogUtility.IsEnable) LogUtility.Debug(string.Format("PDU: {0}", frameLength), "ReadRequestResponse");

			byte[] frame = CollectionUtility.Concat(mbapHeader, messageFrame);
			//_logger.InfoFormat("RX: {0}", StringUtility.Join(", ", frame));
            if (LogUtility.IsEnable) LogUtility.Debug(string.Format("RX: {0}", StringUtility.Join(", ", frame)), "ReadRequestResponse");
		...
		internal override void Write(IModbusMessage message)
		{
            ...
            if (LogUtility.IsEnable) LogUtility.Debug(string.Format("TX: {0}", StringUtility.Join(", ", frame)), "Write");
