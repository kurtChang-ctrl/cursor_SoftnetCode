...
namespace Modbus.IO
{
	...
	internal class ModbusRtuTransport : ModbusSerialTransport
	{
		...
		internal override IModbusMessage ReadResponse<T>()
		{
            ...
            if (LogUtility.IsEnable) LogUtility.Debug(string.Format("RX: {0}", StringUtility.Join(", ", frame)), "ReadResponse");
		...
		internal override byte[] ReadRequest()
		{
			...
			byte[] frame = CollectionUtility.Concat(frameStart, frameEnd);
			//_logger.InfoFormat("RX: {0}", StringUtility.Join(", ", frame));
            if (LogUtility.IsEnable) LogUtility.Debug(string.Format("RX: {0}", StringUtility.Join(", ", frame)), "ReadRequest");
		...
		public static int RequestBytesToRead(byte[] frameStart)
		{
			...
			switch (functionCode)
			{
				...
				default:
                    ...
                    if (LogUtility.IsEnable) LogUtility.Debug(string.Format(errorMessage), "RequestBytesToRead");
		...
		public static int ResponseBytesToRead(byte[] frameStart)
		{
			...
			switch (functionCode)
			{
				...
				default:
					string errorMessage = String.Format(CultureInfo.InvariantCulture, "Function code {0} not supported.", functionCode);
					//_logger.Error(errorMessage);
                    if (LogUtility.IsEnable) LogUtility.Debug(string.Format(errorMessage), "ResponseBytesToRead");
					throw new NotImplementedException(errorMessage);
			}
