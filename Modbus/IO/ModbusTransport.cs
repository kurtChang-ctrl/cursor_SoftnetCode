...
namespace Modbus.IO
{
	...
	public abstract class ModbusTransport : IDisposable
	{
		...
		internal virtual T UnicastMessage<T>(IModbusMessage message, ref string err_MEG) where T : IModbusMessage, new()
		{
			...
			do
			{
						...
						do
						{
							...
							if (exceptionResponse != null)
							{
								...
								if (readAgain = exceptionResponse.SlaveExceptionCode == Modbus.Acknowledge)
								{
                                    ...
                                    if (LogUtility.IsEnable) LogUtility.Debug(string.Format("Received ACKNOWLEDGE slave exception response, waiting {0} milliseconds and retrying to read response.", _waitToRetryMilliseconds), "UnicastMessage");
				...
				catch (SlaveException se)
				{
                    ...
                    if (LogUtility.IsEnable) LogUtility.Debug(string.Format("Received SLAVE_DEVICE_BUSY exception response, waiting {0} milliseconds and resubmitting request.", _waitToRetryMilliseconds), "UnicastMessage");
                    //if (Error != null) { Error(TMError.ModbusError, ("Modbus Error. " + se.Message)); }
                    err_MEG = se.Message;
					Thread.Sleep(WaitToRetryMilliseconds);
				}
				catch (Exception e)
				{
						...
						e is IOException)
					{
                        ...
                        if (LogUtility.IsEnable) LogUtility.Debug(string.Format("{0}, {1} retries remaining - {2}", e.GetType().Name, _retries - attempt + 1, e), "UnicastMessage");

                        if (attempt++ > _retries)
                        {
                            
                            //if (Error != null) { Error(TMError.ModbusError, ("Modbus Error. " + e.Message)); }
                            //###???throw; masterºÝªºerror
                        }
					}
					else
