...
namespace Modbus.Data
{
	...
	public class DataStore
	{
		...
		private readonly object _syncRoot = new object(); // for DataStore read/write
        private readonly static object _ctrlLock = new object(); // for Write command to do something robot controller action
		...
		internal static void WriteData<TData>(DataStore dataStore, IList<TData> items, ModbusDataCollection<TData> destination, ushort startAddress, object syncRoot)
		{
            ...
            // response and handler in background
            System.Threading.Tasks.Task _t = new System.Threading.Tasks.Task(() =>
            {
                lock (_ctrlLock) // locker for robot controller action
                {
