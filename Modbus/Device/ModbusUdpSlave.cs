...
namespace Modbus.Device
{
	...
	public class ModbusUdpSlave : ModbusSlave
	{
		...
		public override void Listen()
		{
            ...
            if (LogUtility.IsEnable) LogUtility.Debug("Start Modbus Udp Server.", "Listen");
			...
			try
			{
				while (true)
				{
                    ...
                    if (LogUtility.IsEnable) LogUtility.Debug(string.Format("Read Frame completed {0} bytes", frame.Length), "Listen");
					//_logger.InfoFormat("RX: {0}", StringUtility.Join(", ", frame));
                    if (LogUtility.IsEnable) LogUtility.Debug(string.Format("RX: {0}", StringUtility.Join(", ", frame)), "Listen");
                    ...
                    if (LogUtility.IsEnable) LogUtility.Debug(string.Format("TX: {0}", StringUtility.Join(", ", responseFrame)), "Listen");
