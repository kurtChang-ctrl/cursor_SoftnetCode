...
";
        ...
        //get dept manager Id
        public static string SqlDeptMgr = "select MgrId from dbo.Dept where DeptId='{0}'";
...
";
        ...
        public static async Task<string> SignRowAsync(string flowSignId, bool signYes,
            string signNote, string sourceTable, bool isTest)
        {
            ...
            var sourceId = row["SourceId"].ToString().Trim();
            if (row == null)
            {
...
";
            ...
            //update source table
            sql = $"select DOCType from dbo.DOCRole where DOCNO='{sourceId.Substring(0, 4)}'";
            var tmp = await db.GetJsonAsync(sql);
            if (tmp == null)
            {
                error = $"not found {signTable} row.(Id={sourceId})";
                goto lab_error;
            }
            switch (tmp["DOCType"].ToString())
            {
                case "1": sourceTable = "dbo.DOC1Buy"; break;
                case "2": sourceTable = "dbo.DOC2Sales"; break;
                case "3": sourceTable = "dbo.DOC3stock"; break;
                case "4": sourceTable = "dbo.DOC3stock"; break;
                case "5": sourceTable = "dbo.DOC3stock"; break;
                case "6": sourceTable = "SoftNetSYSDB,dbo.PP_WorkOrder"; break;
                case "7": sourceTable = "dbo.DOC4Production"; break;
                case "8": sourceTable = "dbo.DOC3stock"; break;
                case "9": sourceTable = "dbo.DOC6Account"; break;
            }
