...
namespace Base.Services
{
    ...
    public class CrudEdit : CrudBase
    {
        ...
        */
        ...
        {
            return _Object.IsEmpty(row[col]) ? -1 : Int32.TryParse(row[col].ToString(), out int num) ? num : 0;
        }
        ...
        private async Task<bool> InsertRowAsync(EditDto edit, JObject inputRow, Db db)
        {
            ...
            if (userId == null || userId=="") { error = $"網頁逾時, 請重新登入網頁"; goto lab_error; }
            foreach (var field in inputRow)
            {
                ...
                if (fid == "ServerId" && (value == null || value.Trim() == "")){ value = _Fun.Config.ServerId; }
                //add keys & values
                fids += fid + ",";
                values += "@" + fid + ",";
                AddArg(fid, value);
        ...
        {
            if (_Fun.UserId() == null || _Fun.UserId() == "") {
                await _Log.ErrorAsync($"網頁逾時, 請重新登入網頁");
                return false;
            }
            //skip if empty
            ...
            */
            ...
            var sql = "";
            var rowKey = "";
            if (edit.PkeyFids_Value != "")
            { rowKey = edit.PkeyFids_Value; }
            else
            { rowKey = inputRow[edit.PkeyFid].ToString(); }
            foreach (var field in inputRow)
            {
                //if no fid then log error !!
                var fid = field.Key;
        ...
        /// <returns>error msg if any</returns>
        private string ValidJson(int editLevel, EditDto edit, JObject json,char type)
        {
            if (json == null)
                return "";
            ...
            if (rows != null)
            {
                ...
                foreach(var row in rows)
                {
                    if (row == null)
                        continue;
                    error = ValidRow(edit, row as JObject,type);
            ...
            for(var i=0; i<childLen; i++)
            {
                ...
                error = ValidJson(editLevel + 1, edit.Childs[i], json2,type);
                if (!_Str.IsEmpty(error))
                    return error;
            }
        ...
        //validate one  row
        //return error msg if any
        private string ValidRow(EditDto edit, JObject row,char type)
        {
            if (_Json.IsEmptyBySkipUnderLine(row))
                return "";

            #region check required & fid existed
            //###??? edit 20220505if (IsNewKey(row, edit.PkeyFid))
            //_Object.IsEmpty(row[col]) ? -1 : Int32.TryParse(row[col].ToString(), out int num) ? num : 0;
            if (type == 'C')
            {
                //check required
        ...
        {
            return await SaveJsonAsync(json, fnSetNewKey, fnAfterSave,'C');
        }
            ...
            FnSetNewKeyJson fnSetNewKey = null, FnAfterSaveAsync fnAfterSave = null)
        {
            ...
            {
                if (_editDto.PkeyFids != null && _editDto.PkeyFids.Length > 0)
                { _editDto.PkeyFids_Value = key; }
            ...
            return await SaveJsonAsync(json, fnSetNewKey, fnAfterSave,'E');
        }
        ...
        private async Task<ResultDto> SaveJsonAsync(JObject inputJson, 
            FnSetNewKeyJson fnSetNewKeyJson = null, FnAfterSaveAsync fnAfterSaveAsync = null,char type = 'E')
        {
            var log = false;
            Db db = null;
            string error="";
            ...
            try
            {
                ...
                //check main row
                //SetFidNo(_edit);
                error = ValidJson(0, _editDto, inputJson, type);
                ...
                error = (fnSetNewKeyJson == null)
                    ? SetNewKeyJson(inputJson, _editDto, type)
                    : fnSetNewKeyJson(this, inputJson, _editDto, type);
                if (_Str.NotEmpty(error))
                    goto lab_error;
            ...
            {
                error = $"程式異常,請將下列原因告知系統管理者, 原因:{ex.Message}";
            }
        lab_error:
            if (db != null)
            {
            ...
            if (log)
                await _Log.ErrorAsync($"CrudEdit.cs SaveJsonAsync() failed: {error}",true);
        ...
        //return error msg if any
        public string SetNewKeyJson(JObject inputJson, EditDto edit,char type)
        {
            return SetNewKeyJson2("0", null, inputJson, edit, type);
        }
        ...
        /// <param name="inputJson">JObject for level0, JArray for level1/2</param>
        /// <returns>error msg if any</returns>
        private string SetNewKeyJson2(string levelStr, JObject upNewKey, JObject inputJson, EditDto edit,char type)
        {
            if (inputJson == null)
                return "";
            ...
            if (inputRows != null)
            {
                ...
                foreach (var inputRow0 in inputRows)
                {
                    ...
                    //IsKeyValueNUM檢查
                    if (kid != "Id" && !edit.IsKeyValueNUM && inputRow[kid] != null )
                    {
                        decimal decimalValue;
                        if (inputRow[kid].ToString().Trim()!="" && decimal.TryParse(inputRow[kid].ToString(), out decimalValue))
                        {
                            error = $"{edit.PkeyFid} 不能是數值.";
                            goto lab_error;
                        }
                    }
                    ...
                    if (HasField(inputRow, kid))
                    {
                        ...
                        if (pkeyIdx != 0)
                        {
                            ...
                            if (kid == "Id")
                            {
                                var key = _Str.NewId('Z');
                                inputRow[kid] = key;
                                upNewKey2["f" + pkeyIdx] = key;
                                inputRow[IsNew] = "1";
                            }
                            else { upNewKey2["f" + pkeyIdx] = inputRow[kid]; }
                            if (type == 'C')
            ...
            for (var i = 0; i < childLen; i++)
            {
                //recursive call
                var childJson = GetChildJson(inputJson, i);
                error = SetNewKeyJson2(levelStr + i, upNewKey2, childJson, edit.Childs[i], type);
                if (_Str.NotEmpty(error))
                    return error;
            }
            #endregion
        ...
        private async Task<bool> DeleteRowsByKeysAsync(EditDto edit, List<string> keys, Db db = null)
        {
            ...
            //set sql args
            var kid = edit.PkeyFid;
            if (edit.PkeyFids != null && edit.PkeyFids.Length > 0)
            {
                string[] dataValue = new string[edit.PkeyFids.Length];
                foreach (string key in keys)
                {
                    string[] tmp = key.Split(';');
                    for (int i = 0; i < tmp.Length; i++)
                    {
                        if (dataValue[i] == null || dataValue[i] == "") { dataValue[i] = $"'{tmp[i]}'"; }
                        else { dataValue[i] = $"{dataValue[i]},'{tmp[i]}'"; }
                    }
                }
                sql = $"delete from {edit.Table} where ";
                for (int i=0;i< dataValue.Length;i++)
                {
                    if (i == 0)
                    { sql = $"{sql} {edit.PkeyFids[i]} in ({dataValue[i]})"; }
                    else
                    { sql = $"{sql} and {edit.PkeyFids[i]} in ({dataValue[i]})"; }
                }
            }
            else
            {
                ...
                sql = string.Format(_Fun.DeleteRowsSql, edit.Table, kid, values[0..^1]);
            }
            //update db
            var count = await db.ExecSqlAsync(sql, _sqlArgs);
            if (count!=0 && edit.HasFlowSign)
            {
                //刪簽核XpFlowSign
                using (DBADO db2 = new DBADO("1", _Fun.Config.Db))
                {
                    foreach (string s in keys)
                    {
                        db2.DB_SetData($"DELETE FROM dbo.[XpFlowSign] where SourceId='{s}'");
                    }
                }
            }
            //if (count == 0)
        ...
        private async Task<List<string>> GetKeysByUpKeysAsync(EditDto edit, List<string> upKeys, Db db)
        {
            if (upKeys == null || upKeys.Count == 0)
                return null;
            var sql = "";
            if (edit.FkeyFids != null)
            {
                sql = string.Format("select {0} from {1} where ", edit.PkeyFid, edit.Table);
                //sql = $"{sql} ";
                if (edit.FkeyFids != null && edit.FkeyFids.Length > 0)
                {
                    string[] dataValue = new string[edit.FkeyFids.Length];
                    ...
                    {
                        if (i == 0)
                        { sql = $"{sql} {edit.FkeyFids[i]} in ({dataValue[i]})"; }
                        else
                        { sql = $"{sql} and {edit.FkeyFids[i]} in ({dataValue[i]})"; }
                    }
                }
            }
            else
            { sql = string.Format("select {0} from {1} where {2} in ({3})", edit.PkeyFid, edit.Table, edit.FkeyFid, _List.ToStr(upKeys, true)); }
