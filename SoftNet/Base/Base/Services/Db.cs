...
namespace Base.Services
{
    ...
    public class Db : IAsyncDisposable
    {
        ...
        private const int _dbSec = 2000; //command second, can not too short 原600 暫時改 2000
        ...
        public async Task<bool> InitDbAsync()
        {
            ...
            catch (Exception ex)
            {
                ...
                await _Log.ErrorAsync($"資料庫 connect db error: Message={ex.Message} StackTrace={ex.StackTrace}",true);
                return false;
            }
        }
        ...
        */
        ...
        private async Task<bool> InitCmdAsync(string sql, List<object> sqlArgs = null, int dbSec = _dbSec)
        {
            ...
            if (sqlArgs != null && sqlArgs.Count > 0)
            {
                ...
                for (var i = 0; i < sqlArgs.Count; i += 2)
                {
                    ...
                    Type _s = sqlArgs[i + 1].GetType();

                    if (sqlArgs[i + 1] == null)
                    { arg.Value = DBNull.Value; }
                    else if (sqlArgs[i + 1].GetType() == typeof(decimal) || decimal.TryParse(sqlArgs[(i + 1)].ToString(),out redecimal))
                    {
                        arg.Value = sqlArgs[i + 1];
                    }
                    else if (sqlArgs[i + 1].GetType() == typeof(DateTime) || DateTime.TryParse(sqlArgs[(i + 1)].ToString(), out result))
                    {
                        ...
                        //arg.ParameterName = sqlArgs[i].ToString();
                        arg.Value = result.ToString("MM/dd/yyyy HH:mm:ss.fff");
                        //arg.DbType = DbType.DateTime;
                    }
                    else
        ...
        public async Task<int> GetJsonAsync_ReturnCount(string sql, List<object> sqlArgs = null)
        {
            var rows = await GetJsonsAsync(sql, sqlArgs);
            return (rows == null || rows.Count == 0) ? 0 : rows.Count;
        }
        ...
        public JObject ReaderGetJson(IDataReader reader)
        {
            ...
            for (var i = 0; i < _colMap.Count; i++)
            {
                ...
                row[fid] = reader.IsDBNull(i) ? "" :
                    (type == DataTypeEnum.Datetime) ? reader.GetDateTime(i).ToString("yyyy-MM-dd HH:mm:ss.fff") :
