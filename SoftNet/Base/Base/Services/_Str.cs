...
namespace Base.Services
{
    public class _Str
    {
		...
		private static object lock_NewId = new object();
        ...
        */
        ...
        public static bool Set_Simulation_flag=false;//是否計算排程中
        public static bool Get_Simulation_flag()
        {
            return Set_Simulation_flag;
        }
        ...
        /// </summary>
        /// <returns></returns>
        public static string NewId(char type) //type=Z(資料Id) Y=(排程流程) X=NeedData碼 O=logId
        {
			...
			lock (lock_NewId)
            {
                ...
                data = $"{type}{_Fun.Config.ServerId}{data}";
            }
            return data;
        }
        ...
        public static async Task<bool> CheckKeyAsync(string str, bool logError = true)
        {
            ...
            //if (rg.IsMatch(str))
            //    return true;
            if (!isContainSpace(str)) { return true; }

            if (logError)
                await _Log.ErrorAsync($"_Str.CheckKeyRuleAsync() failed ({str})");
            return false;
        }
        //檢查TextBox輸入文字是否有包含下列字元
        public static bool isContainSpace(string strtxtBox)
        {
            if (strtxtBox.Trim() == "") { return true; }
            bool result = false;
            string[] arrSynax = new string[] { "+", "*", "%", "'", "@", "\"", "?", ">", "<", "|", "&", "\\" };//###???暫時拿掉" ", ":", "-", "/", "." 因日期的關希

            for (int i = 0; i < arrSynax.Length; i++)
            {
                //第一階段檢查
                if (strtxtBox.LastIndexOf(arrSynax[i])>=0)
                {
                    result = true;  //有包含下列字元
                    break;
                }
            ...
            /*  //###??? 20220604拿掉 改EditDto IsKeyValueNUM設定
            //第二階段檢查
            if (result == false)
            {
                result = isAllNum(strtxtBox);
            }
            */
            return result;
        }
        private static bool isAllNum(string strtxtBox)
        {
            bool result = false;
            decimal decimalValue;
            result = decimal.TryParse(strtxtBox, out decimalValue);
            return result;
        }
