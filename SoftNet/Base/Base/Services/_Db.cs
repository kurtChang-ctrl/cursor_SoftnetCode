...
namespace Base.Services
{
    ...
    public class _Db
    {
        ...
        public static async Task<List<T>> GetModelsAsync<T>(string sql, List<object> args = null, string dbStr = "")
        {
            ...
            Db db= new Db(dbStr);
            List<T> rows= await db.GetModelsAsync<T>(sql, args);
        ...
        {
            Db db = new Db(dbStr);
            var row=db.GetStrAsync(sql, args);
            await db.DisposeAsync();
            return await row;
        ...
        {
            Db db = new Db(dbStr);
            var row = db.GetStrsAsync(sql, args);
            await db.DisposeAsync();
            return await row;
        ...
        {
            Db db = new Db(dbStr);
            var row = db.ExecSqlAsync(sql, args);
            await db.DisposeAsync();
            return await row;
        ...
        {
            Db db = new Db(dbStr);
            var row = db.SetRowStatus(table, kid, kvalue, status, statusId, where);
            await db.DisposeAsync();
            return await row;
