    ...
    public class CrudBase
    {
        ...
        protected string GetWhereAndArg(EditDto edit, string key)
        {
            ...
            if (edit.PkeyFids != null && edit.PkeyFids.Length > 0)
            {
                ...
                for (int i=0;i< edit.PkeyFids.Length;i++)
                {
                    AddArg(edit.PkeyFids[i], tmp[i]);
                    if (re == "")
                    { re = $"{edit.PkeyFids[i]}=@{edit.PkeyFids[i]}"; }
                    else
                    { re = $"{re} and {edit.PkeyFids[i]}=@{edit.PkeyFids[i]}"; }
                }
                return re;
            }
            else
        ...
        protected string GetSqlByField(EditDto edit, string key)
        {
            if (key.IndexOf(';')>=0)
            {
                ...
                string re = edit.ReadSql;
                for (int i=0;i< keys.Length;++i)
                {
                    re = re.Replace("{"+i.ToString()+"}", keys[i]);
                }
                return re;
            }
        ...
        /// <param name="db"></param>
        /// <returns></returns>
        public async Task<JObject> GetDbRowAsync(EditDto edit, string key, Db db = null,bool dbIsClose=true)
        {
            //reset sqlArgs first
            //ResetArg();
            ...
            //var row = db.DB_TestROW(sql, _sqlArgs);
            //if (dbIsClose) { db.Dispose(); }
        ...
        protected async Task<JObject> GetJsonAsync(CrudEnum crudEnum, string key)
        {
            ...
            //{
                data = await GetDbRowAsync(_editDto, key, db,false);    //return data
        ...
        protected async Task<JObject> GetChildDbJsonAsync(int editLevel, EditDto edit, List<string> keys, Db db)
        {
            ...
            //var rows = db.DB_TestROWByArray(sql, null);
