...
namespace Base.Services
{
    ...
    public class CrudRead
    {
        ...
        public async Task<JObject> GetPageAsync(ReadDto readDto, DtDto dtDto, string ctrl = "")
        {
            ...
            JArray rows = null;
            var rowCount = 0;
            if (readDto.SQL_StoredProgram != "")
            {
                #region 預存程序
                //rows = await db.GetJsonsAsync($"{readDto.SQL_StoredProgram} {dtDto.start},{dtDto.length}");
                using (DBADO tmp_db = new DBADO("1", _Fun.Config.Db))
                {
                    rows = tmp_db.DB_Test($"{readDto.SQL_StoredProgram} {dtDto.start},{dtDto.length}", null);
                    if (dtDto.recordsFiltered < 0 && rows != null)
                    {

                        DataRow dr = tmp_db.DB_GetFirstDataByDataRow($"{readDto.SQL_StoredProgram} 0,0");
                        if (dr != null && !dr.IsNull(0)) { rowCount = int.Parse(dr[0].ToString()); }

                    }
                    else { rowCount = dtDto.recordsFiltered; }
                }
                #endregion
            }
            else if (readDto.SQL_ByProgram != null && readDto.SQL_ByProgram.rows != null)
            {
                #region 自訂多組合sql語法
                rows = readDto.SQL_ByProgram.rows;
                if (dtDto.recordsFiltered < 0 && rows != null)
                { rowCount = readDto.SQL_ByProgram.RowCount; }
                else { rowCount = dtDto.recordsFiltered; }
                if (readDto.SQL_ByProgram != null) { readDto.SQL_ByProgram.Dispose(); }
                #endregion
            }
            else
            {
                ...
                //JArray rows = null; //將Project DB table放入,不知要幹嘛
                //var db = GetDb();
                rowCount = dtDto.recordsFiltered;
                ...
                if (rowCount < 0)
                {
                    if (sqlDto.From.IndexOf("join ") > 1 && group != "")
                    {
                        sql = "Select Count(*) as _count " +
                            sqlDto.From + " " +
                            sqlDto.Where +
                            group;
                        rowCount = await db.GetJsonAsync_ReturnCount(sql, _sqlArgs); //for log carrier
                        if (rowCount == 0)
                        {
                            rowCount = 0;
                            goto lab_exit;
                ...
                */
                ...
                using (DBADO tmp_db = new DBADO("1", _Fun.Config.Db))
                {
                    rows = tmp_db.DB_Test(sql, _sqlArgs);
                    if (rowCount < 0)
                    {
                        if (readDto.SpecifySQLFrom!="")
                        {
                            sql = readDto.SpecifySQLFrom;
                            //var row = await db.GetJsonAsync(sql, _sqlArgs); //for log carrier
                            var row = tmp_db.DB_TestROW(sql, _sqlArgs);
                            if (row == null)
                            {
                                rowCount = 0;
                                goto lab_exit;
                        ...
                        else if (sqlDto.From.IndexOf("join ") > 1 && group != "")
                        {
                            sql = $"select {sqlDto.Select} {sqlDto.From} {sqlDto.Where} {(sqlDto.Group == "" ? "" : sqlDto.Group)}";
                            rowCount = tmp_db.DB_TestQueryCount(sql, _sqlArgs);
                            if (rowCount == 0)
                            {
                                rowCount = 0;
                                goto lab_exit;
            ...
            if (items != null && items.Length > 0 && findJson != null)
            {
                ...
                foreach (var prop in findJson)
                {
                    ...
                    if (item == null)
                    {
                        ...
                        var key = $"{readDto.Db_forFind_From_AS_Name}{prop.Key}";
                        var len = key.Length;
                    ...
                    if (readDto.Db_forFind_From_AS_Name != "")
                    { col = $"{readDto.Db_forFind_From_AS_Name}{col}"; }
